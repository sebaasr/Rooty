...
export function Pill({ label, active, onClick, size = "md" }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      fontSize: sm ? 11 : 12, fontWeight: 600, fontFamily: "DM Sans, sans-serif",
...
export function Badge({ label, color = "blue" }: { label: string; color?: "blue" | "gold" | "green" | "red" | "gray" }) {
  ...
  return (
    <span style={{
      ...
      background: s.bg, color: s.color, fontFamily: "DM Sans, sans-serif", whiteSpace: "nowrap" as const,
    }}>{label}</span>
  )
}
...
export function Btn({ children, variant = "primary", onClick, fullWidth, size = "md", disabled, style: extra = {} }: {
  ...
  const base: React.CSSProperties = {
    ...
    width: fullWidth ? "100%" : "auto", transition: "all .15s",
    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6,
    opacity: disabled ? 0.5 : 1,
  }
  const variants = {
    primary: { background: "var(--blue)", color: "#fff" },
    gold: { background: "var(--gold)", color: "#1C1C1C" },
    outline: { background: "transparent", color: "var(--blue)", border: "1.5px solid var(--blue)" },
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? "0 8px 24px rgba(0,48,135,0.12)" : "0 2px 8px rgba(0,0,0,0.06)",
...
export function SectionHeader({ title, action, onAction }: { title: string; action?: string; onAction?: () => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
      <span style={{ fontFamily: "Lora, serif", fontWeight: 700, fontSize: 16, color: "var(--text)" }}>{title}</span>
      {action && (
...
export function EmptyState({ icon, title, sub }: { icon: string; title: string; sub: string }) {
  return (
    <div style={{ textAlign: "center", padding: "48px 24px", color: "var(--muted)" }}>
      ...
      <div style={{ fontFamily: "Lora, serif", fontWeight: 600, fontSize: 16, color: "var(--text)", marginBottom: 6 }}>{title}</div>
    ...
    <Card style={{ padding: "20px 22px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: c.light, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Icon name={icon} size={18} color={ic} strokeWidth={1.75} />
        </div>
        {trend && <Badge label={trend} color={trendColor as "green" | "gray"} />}
      </div>
      <div style={{ fontFamily: "Lora, serif", fontWeight: 700, fontSize: 28, color: "var(--text)", lineHeight: 1 }}>{value}</div>
